...
async function callBool(selector, ownerBase58, contractBase58, addressBase58) {
  ...
  const out = await triggerConstant({ contract: contractBase58, selector, paramHex64: param, ownerBase58 });
...
app.get("/api/health", (req, res) => {
  res.json({ ok: true, TRONGRID_API, usdt_contract: USDT });
});
...
app.get("/api/wallet/:address/summary", async (req, res) => {
  try {
    ...
    if (!isTronBase58(address)) return res.status(400).json({ error: "Invalid TRON address (starts with T)" });
...
app.listen(PORT, "0.0.0.0", () => console.log(`SEKURA – MVP running on http://localhost:${PORT}`));
